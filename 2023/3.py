...
for i, line in enumerate(lines):
    symbols = []
    if i > 0:
        symbols += [x.start() for x in re.finditer("[^0-9.]", lines[i-1])]
    next_symbols = None
    if i < len(lines)-1:
        symbols += [x.start() for x in re.finditer("[^0-9.]", lines[i+1])]
    symbols += [x.start() for x in re.finditer("[^0-9.]", line)]
...
for i, line in enumerate(lines):
    numbers = []
    if i > 0:
        numbers += [[x, i-1] for x in re.finditer(r'\d+', lines[i-1])]
    next_symbols = None
    if i < len(lines)-1:
        numbers += [[x, i+1] for x in re.finditer(r'\d+', lines[i+1])]
    numbers += [[x, i] for x in re.finditer(r'\d+', line)]
    gears = re.finditer('\*', line)
    for gear in gears:
        adjecent_numbers = []
        start = gear.start()
        for num, line_index in numbers:
            num_start = num.start()
            num_end = num.end()

            if start >= num_start-1 and start <= num_end:
                # print("here")
                # try:
                #     print("-----------")
                #     # print(lines[i-1])
                ...
                # except:
                #     pass
                adjecent_numbers.append(lines[line_index][num_start:num_end])

        if len(adjecent_numbers) == 2:
            print(adjecent_numbers)
            res += int(adjecent_numbers[0]) * int(adjecent_numbers[1])
print(res)
